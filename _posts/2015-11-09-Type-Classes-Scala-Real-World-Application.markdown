...
the second line DistanceLikeForShort was plumbed in

## Simpler type class notation
This code is a little verbose though. It turns out we can simplifythe way to pass the distanceLike around. The method isSumOfDistancesLessThen below shows how this can be done.  This new syntax below is quite exciting in that the method or class signatures read a lot better. 
...
During this profiling work a constant worry is about memory usage. There will be millions of these Distance objects around, and there will be half a billion calls involving them. In Java any time that you put a short or an int into 
a generic data structure, then it needs to wrapped with a Box object of type Short or Integer. Scala has a very very nice feature called AnyVals that means that at run time variables of type Int or Short vanish and
...
These include the following (D will be something with a Distance type class. In practice it will be an Int or a Short.
{% highlight scala %}
  Map[(Int, Int), D]
  Array[Array[D]]
{% endhighlight %}
Other types will be used as well, but just looking at these three will show the basics.
...
(where D has Distance like behaviour). To deal with that we produce classes that are parameterised by 'D:Distance'.  It turns out that creating Arrays of type D in Scala requires plumbing, and this turned out to be a very easy way of doing that plumbing.
...
explosion of needing a separate version of this for each combination of Matrix and Distance. As the work progressed, it was found necessary to break this pattern as Java classes
...
The Floyd Warshell algorithm is simple to use and understand and does exactly what is needed. I did have to edit the wiki page (a very scary moment) as there was an error on it involved a
 ...
 Unfortunately as we shall see the really nice 'D: Distance' notation cannot be used with Matrixes. It would be nice if we could have MileageFactory[D: Distance: Matrix[D]], but Scalas as 
...
abstract class MileageFactory[D: Distance, MM](implicit matrixLike: Matrix[D, MM]) {
  ...
  protected def createAndInitialiseMatrix(locationSize: Int, edges: Traversable[MileageEdge[D]]) = {
    ...
    val withLocToLocBeingZero = locs.foldLeft(matrixLike.makeRaw(locs.size, large))(
      (hs, loc) => matrixLike.put(hs, loc, loc, zero))
    edges.foldLeft(withLocToLocBeingZero) {
      case (mm, MileageEdge(from, to, distance)) =>
        matrixLike.put(matrixLike.put(mm, from, to, zero), to, from, zero)
    }
  }
}
...
{% endhighlight %}
The code has been split up as several versions of MileageFactory are going to be evaluated. Making the initial value takes very little time, so there is little point varying it. Almost all the time is in the
three deep nested loop of k, i and j.
...
The three mileage factories (MF1, MF2 and MF3) have different ways of writing the really tight loops in Scala. The implementations are important (but can be seen in the Git Repository), what is
interesting is that there was about a 30% time saving. It depends on the application, and how much time the loop takes as to whether the rewriting is worth it

ALl times are in in miliseconds. The using time is how long it will take to do half a billion random accesses
...
Because of the ease of adding new Matrix types, I actually have a few other representations too. The last two (ShortArrayAndLength and IntArrayAndLength) are both Java classes using
...
Some of the Matrix like classes I created were native Java using short[] and int[]. They were just faster. I rewrote the MatrixFactory in 
...
so the Just in time compiler could in-line everything.

As an aside, when I had finished this I tried using native Java code not using iterators that had no inheritance. It ran perhaps an order of magnitude faster than any of these approaches.
Passing closures, or functions, into the heart of loops that are to run fast doesn’t work for high performance code!
...
I predicted incorrectly which would work better: an Array[Array[Int]] or Array[Int] with calculations. I thought the second would be going through less memory locations. 
...
the method calls, but that’s just a guess. 
...
is much faster. This data structure I saved to a file, so that when running the batch job I just had to read the file and populate the data structure. This took a few seconds instead of the minute or so that the 
...
eight cores the 300 seconds still took 200 seconds, whereas with the new structure it took about 120s.  
...
personally gained a much better idea of how the data structure and access affected execution times.
